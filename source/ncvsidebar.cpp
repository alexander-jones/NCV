...
void NCVSidebar::setTabWidget(const QString& string, NCVElementType element, QWidget * widget, bool show )
{
    if (m_taskPanels.contains(string))
    {
        if (element == Layer)
        {
            ...
            m_taskPanels[string]->insertTask(0,widget,QIcon(":/assets/layerIcon.png"),"Layers");
        }
        else if (element == Column)
        {
            m_taskPanels[string]->removeTask(1);
            m_taskPanels[string]->insertTask(1,widget,QIcon(":/assets/columnIcon.png"),"Column");
        }
        else if (element == Neuron)
        {
            m_taskPanels[string]->removeTask(2);
            m_taskPanels[string]->insertTask(2,widget,QIcon(":/assets/neuronIcon.png"),"Neurons");
        }
        else if (element == Connection)
        {
            m_taskPanels[string]->removeTask(3);
            m_taskPanels[string]->insertTask(3,widget,QIcon(":/assets/connectionIcon.png"),"Connections");
        }
        if (show)
            widget->show();
    }
