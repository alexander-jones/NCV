...
uniform int BitsPerValue, States, BitsVal;
...
void main( void )
{
    ...
    Vert_Value = clamp(uint(texelFetch(Inst_Attribute,startOffset/8).r * 255.0) & (uint(BitsVal) << uint(startOffset % 8)),0U,uint(States-1));
    int endOffset = startOffset + BitsPerValue;
    if ((endOffset % 8) < (startOffset % 8))
        Vert_Value += clamp(uint(texelFetch(Inst_Attribute,endOffset/8).r * 255.0) & (uint(BitsVal) >> uint(BitsPerValue - (endOffset % 8))),0U,uint(States-1));
    Vert_ID = Inst_ID;
