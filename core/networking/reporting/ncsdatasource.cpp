...
NCSDataSource::NCSDataSource(QObject *parent) : QObject(parent)
{
    m_neurons = NULL;
    m_connections = NULL;
}
...
void NCSDataSource::replaceNeuronSet(NCSNeuronSet *neurons)
{
    ...
    if(m_neurons)
    {
        ...
        for(QMap<QString, NCSAttribute*>::iterator it = attributes.begin(); it != attributes.end(); it++)
        {
            if(it.value()->reportable())
                addAttribute(it.key(), it.value(), m_neurons->count());
        }
    }
}
...
void NCSDataSource::replaceConnectionSet(NCSConnectionSet *connections)
{
    QMap<QString, NCSAttribute*> attributes;
    ...
    if(m_connections)
    {
        ...
        {
            if(it.value()->reportable())
                addAttribute(it.key(), it.value(), m_connections->count());
        }
    }
}
...
void NCSDataSource::updateAttribute(NCSAttribute *attribute)
{
    ...
    if(reportedAttribute.attribute->type() == Continuous)
    {
        ...
        contAttr->attachData(QVector<float>::fromStdVector(values));
    }
...
void NCSDataSource::updateAttributes()
{
    if(NULL != m_neurons)
    {
...
void NCSDataSource::addAttribute(const QString& name, NCSAttribute *attribute, int elementCount)
{
